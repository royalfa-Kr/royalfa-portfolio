...
export default function SMDApp() {
  ...
  // Calcular valor actual basado en los exponentes reales (no solo el índice de la tabla)
  const initialExp = METRIC_PREFIXES[initialIndex].exp;
  const currentExp = METRIC_PREFIXES[currentIndex].exp;
  const currentValue = baseValue * Math.pow(10, initialExp - currentExp);
  ...
  const generateExercise = (cat = exerciseCategoryFilter, diff = exerciseDifficulty) => {
    ...
    // Generar respuesta del ejercicio usando diferencia de exponentes
    const fromExp = METRIC_PREFIXES[fromIdx].exp;
    const toExp = METRIC_PREFIXES[toIdx].exp;
    const correctAnswer = val * Math.pow(10, fromExp - toExp);
          ...
          <section className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3 w-full md:w-auto">
                <button onClick={() => moveStep(-1)} disabled={currentIndex === 0} className="flex-1 md:flex-initial bg-slate-900 hover:bg-indigo-900 text-white disabled:opacity-40 px-5 py-3 rounded-xl font-bold flex items-center justify-center gap-2 group text-sm">
                  <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform text-indigo-400" /> <span>Múltiplos Mayores</span>
                </button>
                <button onClick={() => moveStep(1)} disabled={currentIndex === METRIC_PREFIXES.length - 1} className="flex-1 md:flex-initial bg-indigo-600 hover:bg-indigo-700 text-white disabled:opacity-40 px-5 py-3 rounded-xl font-bold flex items-center justify-center gap-2 group text-sm">
                  <span>Submúltiplos Menores</span> <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            ...
            <div className="bg-slate-950 rounded-2xl p-5 shadow-2xl overflow-hidden border border-slate-800">
              <div className="flex justify-between items-center mb-3 text-xs border-b border-slate-800/80 pb-3">
                <span className="flex items-center gap-1 font-bold text-indigo-400"><ArrowLeftCircle className="w-4 h-4"/> Mayores</span>
                <span className="font-black text-slate-300 uppercase tracking-widest hidden md:inline">Tabla de Conversión Oficial SI</span>
                <span className="flex items-center gap-1 font-bold text-purple-400">Menores <ArrowRightCircle className="w-4 h-4"/></span>
              </div>
              <button onClick={() => { setCurrentIndex(initialIndex); playSound('step'); }} className="px-4 py-2.5 text-slate-600 hover:bg-slate-100 rounded-xl transition flex items-center gap-1.5 text-xs font-bold border border-slate-200">
                <RotateCcw className="w-4 h-4 text-indigo-600" /> <span>Volver a Origen</span>
              </button>
            ...
            <div className="bg-indigo-50 rounded-2xl p-5 border border-indigo-200 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="bg-indigo-600 text-white text-[11px] font-black px-2.5 py-0.5 rounded-full">Factor: 10^{Math.abs(METRIC_PREFIXES[initialIndex].exp - METRIC_PREFIXES[currentIndex].exp)}</span>
              </div>
              ...
              <div className="text-sm text-slate-800 leading-relaxed font-medium">
                {currentIndex === initialIndex ? (
                  ...
                  <>
                    Convertir de <strong>{METRIC_PREFIXES[initialIndex].key}{baseUnit}</strong> a <strong>{METRIC_PREFIXES[currentIndex].key}{baseUnit}</strong> requiere 
                    <span className="bg-indigo-100 text-indigo-900 px-2 rounded mx-1">mover el punto decimal {Math.abs(METRIC_PREFIXES[initialIndex].exp - METRIC_PREFIXES[currentIndex].exp)} lugares a la {METRIC_PREFIXES[initialIndex].exp > METRIC_PREFIXES[currentIndex].exp ? 'DERECHA' : 'IZQUIERDA'}</span>.
                    <br/>Cálculo: {baseValue} {METRIC_PREFIXES[initialIndex].exp > METRIC_PREFIXES[currentIndex].exp ? '×' : '÷'} 10<sup>{Math.abs(METRIC_PREFIXES[initialIndex].exp - METRIC_PREFIXES[currentIndex].exp)}</sup> = 
              ...
              <div className="bg-white rounded-xl p-3 border border-indigo-100 flex items-center gap-3 text-xs font-semibold text-slate-700">
                ...
                <span>Corrimiento Decimal: {currentIndex === initialIndex ? 'Sin movimiento' : `${Math.abs(METRIC_PREFIXES[initialIndex].exp - METRIC_PREFIXES[currentIndex].exp)} lugares a la ${METRIC_PREFIXES[initialIndex].exp > METRIC_PREFIXES[currentIndex].exp ? 'DERECHA' : 'IZQUIERDA'}`}.</span>
              </div>
            </div>
          </section>
        )}
