...
export default function ClasesLandingPage() {
  ...
  const [cargandoDinamicas, setCargandoDinamicas] = useState(true);

  useEffect(() => {
    const fetchReviews = async () => {
      // PREVENCIÓN DE ERROR: Si estamos en el entorno de previsualización (blob URL),
      // cancelamos el fetch relativo que causa el error "Failed to parse URL" y usamos locales.
      if (typeof window !== 'undefined' && window.location.href.startsWith('blob:')) {
        console.warn("Entorno de vista previa detectado. Usando reseñas locales automáticamente.");
        setCargandoDinamicas(false);
        return;
      }

      try {
        // CORRECCIÓN: Usar una marca de tiempo para obligar a Next.js a no cachear la URL en producción
        const timestamp = new Date().getTime();
        const res = await fetch(`/apps/api/testimonios?t=${timestamp}`, {
          method: 'GET',
          cache: 'no-store',
          headers: {
        ...
        if (res.ok) {
          ...
          // Validamos que haya datos antes de cambiar el estado
          if (data && Array.isArray(data) && data.length > 0) {
            setResenasDinamicas(data);
            setUsarLocales(false);
          } else {
            console.warn("API de testimonios respondió, pero la lista está vacía. Usando locales.");
          }
        } else {
          console.error(`Error de API: Código ${res.status}`);
        }
      } catch (error) {
        console.error("Fallo al conectar con Upstash/API. Fallback a locales activado:", error);
      } finally {
        setCargandoDinamicas(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      {/* RESTAURADO: 3. EL DIFERENCIADOR (Laboratorio Virtual) */}
      ...
      <section id="resenas" className="py-16 bg-slate-900 border-b border-slate-800 overflow-hidden">
        <div className="max-w-6xl mx-auto px-6 mb-10 text-center flex flex-col items-center">
          ...
          <p className="text-slate-400 text-sm mb-6">
             {usarLocales && !cargandoDinamicas ? "(Mostrando reseñas destacadas)" : "Reseñas reales de años de experiencia transformando resultados."}
          </p>

          {/* CORRECCIÓN 1: Restaurar el botón de Dejar una reseña */}
          <Link href="apps/testimonios" className="inline-flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white px-5 py-2.5 rounded-full text-sm font-medium transition-all shadow-lg">
            <MessageSquareQuote size={18} className="text-yellow-500" /> Dejar una reseña
          </Link>
        </div>
              ...
              return (
                <div key={index} className="w-80 md:w-96 flex-shrink-0 bg-slate-950 border border-slate-800 rounded-2xl p-6 flex flex-col transition-colors hover:border-slate-700">
                  {tieneTexto ? (
                    ...
                    /* CORRECCIÓN 3: Restaurar estrellas en reseñas sin texto */
